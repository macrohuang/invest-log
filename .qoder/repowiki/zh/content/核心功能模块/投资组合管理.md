...
- 模板渲染：templates/holdings.html（前端展示）
- 配置：config.py（数据库路径等）
...
B --> E["templates/holdings.html<br/>持有明细页面"]
A --> F["routers/utils.py<br/>模板引擎初始化"]
A --> G["config.py<br/>应用配置"]
```
...
- 路由与页面：/holdings、/symbol/{symbol}、价格更新、快速交易、价值调整
...
图表来源
- [database.py](file://database.py#L393-L463)
- [database.py](file://database.py#L874-L884)
...
- templates/holdings.html 依赖后端传入的数据进行渲染
...
HOLD --> PF
DB --> CFG["config.py"]
HOLD --> TPL["templates/holdings.html"]
```
...
## 结论
本系统通过清晰的交易类型定义与 SQL 聚合，实现了准确的多币种投资组合管理。get_holdings() 提供基础持仓与成本均价，get_holdings_by_symbol() 与 get_holdings_by_currency() 则分别从“符号维度”和“资产类型维度”提供可视化与配置监控。adjust_asset_value() 与 INCOME 交易类型确保了价值调整与现金流入的规范记录。配合多源价格抓取与缓存，系统在易用性与准确性之间取得良好平衡。
