...
## 更新摘要
**变更内容**
- 新增数据库配置管理功能，支持数据库文件名和iCloud同步设置
- 增强资产类型管理，提供动态资产类型添加、删除和使用状态检查
- 改进账户管理功能，增强删除操作的安全性验证
- 完善iCloud集成配置，提供完整的多设备同步支持
- 更新设置页面的标签页结构，支持四个主要功能区域
...
**更新** 新增数据库配置管理、iCloud集成和资产类型管理等重大功能增强
...
## 核心组件
系统设置路由包含以下核心组件：
- 设置页面渲染：支持分配设置、账户管理、资产类型管理和数据库配置四个标签页
- 数据持久化：通过SQLite数据库存储配置信息
- 验证规则：确保配置值在有效范围内且满足约束条件
- 安全控制：防止误删正在使用的账户和资产类型
- iCloud集成：自动同步数据库文件到iCloud
- 动态资产类型管理：支持资产类型的添加、删除和使用状态检查

**更新** 新增数据库配置管理和动态资产类型管理功能

**章节来源**
- [routers/settings.py](file://routers/settings.py#L11-L64)
- [database.py](file://database.py#L1087-L1201)
- [config.py](file://config.py#L14-L55)
...
Router-->>Client : 重定向响应
Client->>Router : POST /settings/database
Router->>Config : 更新用户配置
Config-->>Router : 配置保存结果
Router-->>Client : 重定向响应
```

**图表来源**
- [routers/settings.py](file://routers/settings.py#L12-L118)
- [database.py](file://database.py#L827-L882)
- [config.py](file://config.py#L21-L42)
...
+database_settings_submit() RedirectResponse
...
SettingsRouter --> ConfigModule : 读取/写入
SettingsRouter --> LoggerModule : 记录
DatabaseModule --> ConfigModule : 存储
```

**图表来源**
- [routers/settings.py](file://routers/settings.py#L12-L180)
- [database.py](file://database.py#L1087-L1201)
- [config.py](file://config.py#L21-L55)

**章节来源**
- [routers/settings.py](file://routers/settings.py#L12-L180)
- [templates/settings.html](file://templates/settings.html#L15-L266)
...
- 配置冲突检测：避免无效的配置组合
- 数据库文件名验证：防止特殊字符和非法路径

#### 安全控制措施
- 输入过滤：防止SQL注入和XSS攻击
- 权限控制：基于账户使用状态的删除权限
- 操作审计：记录所有配置变更操作
- iCloud路径验证：确保目录存在性和可写权限

**更新** 新增数据库配置验证和iCloud路径安全检查

**章节来源**
- [database.py](file://database.py#L842-L882)
- [database.py](file://database.py#L1124-L1156)
- [database.py](file://database.py#L1194-L1201)
...
**更新** 新增的数据库配置管理和资产类型管理功能进一步增强了系统的灵活性和可扩展性
...
3. **监控告警**：设置日志监控和异常告警
4. **版本升级**：遵循数据库迁移规范
5. **资产类型管理**：谨慎删除正在使用的资产类型
6. **数据库配置**：定期检查数据库文件完整性
7. **iCloud同步**：确保iCloud服务稳定可用
...
3. **审计日志**：记录所有敏感操作
4. **数据加密**：敏感数据传输和存储加密
5. **资产类型安全**：防止删除仍在使用的资产类型
6. **数据库安全**：定期检查数据库文件权限
7. **iCloud安全**：确保iCloud同步过程中的数据安全

**更新** 新增资产类型管理和数据库配置相关的安全建议