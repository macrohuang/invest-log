...
## 简介
本文件面向“投资日志管理系统”的数据库层，系统采用SQLite作为本地存储，结合FastAPI提供Web接口与模板页面。数据库围绕交易记录、账户、资产类别、符号信息以及最新价格等维度进行建模，支持多币种（CNY/USD/HKD）、成本均价计算、持仓状态管理、实时价格缓存与多级查询过滤。本文从表结构、主外键与索引设计、业务规则实现、数据访问模式、查询优化、缓存机制、迁移与备份、性能监控等方面进行全面阐述，并给出可直接参考的SQL示例路径与操作模式。
...
- [price_fetcher.py](file://price_fetcher.py#L325-L401)
- [database.py](file://database.py#L838-L858)
- [database.py](file://database.py#L792-L814)
...
图表来源
- [database.py](file://database.py#L90-L99)
- [database.py](file://database.py#L80-L88)
- [database.py](file://database.py#L344-L390)
...
章节来源
- [database.py](file://database.py#L142-L151)
...
## 结论
该数据库设计以交易为核心，围绕多币种、成本均价与持仓状态构建完整闭环，配合latest_prices缓存与丰富索引提升查询效率。通过清晰的约束与校验、完善的日志与错误处理，系统在易用性与可靠性之间取得良好平衡。建议后续持续完善迁移脚本、监控指标与自动化备份流程，进一步增强生产可用性。

## 附录：常用SQL与操作模式

- 初始化数据库（创建表与索引）
  - 参考路径：[database.py](file://database.py#L22-L151)

- 新增交易（含Cash联动）
  - 参考路径：[database.py](file://database.py#L158-L225)
  - 调用示例（路由）：[routers/transactions.py](file://routers/transactions.py#L45-L74)

- 更新交易（自动重算总额）
  - 参考路径：[database.py](file://database.py#L238-L276)

- 查询交易（多条件过滤与分页）
  - 参考路径：[database.py](file://database.py#L294-L341)
  - 调用示例（API）：[routers/api.py](file://routers/api.py#L18-L31)

- 计算持仓与成本均价
  - 参考路径：[database.py](file://database.py#L344-L390)
  - 组合视图（按币种/资产类型占比与告警）
  - 参考路径：[database.py](file://database.py#L732-L785)

- 获取最新价格与批量缓存
  - 参考路径：[database.py](file://database.py#L838-L883)
  - 调用示例（路由）：[routers/holdings.py](file://routers/holdings.py#L102-L147)

- 资产类型与分配设置
  - 参考路径：[database.py](file://database.py#L890-L960)
  - 参考路径：[database.py](file://database.py#L674-L730)

- 日志记录（价格更新/失败）
  - 参考路径：[database.py](file://database.py#L792-L831)
